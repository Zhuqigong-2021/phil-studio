...
test("root layout mounts one global accessible favorite toast host", async () => {
  ...
  assert.match(host, /RETIRE_AFTER_MS = 320/);
  ...
  assert.match(globals, /transform:\s*translateY\(14px\)/);
  assert.match(globals, /transform:\s*translateY\(-10px\) scale\(0\.96\)/);
  assert.match(globals, /filter:\s*blur\(3px\)/);
  assert.match(globals, /transform:\s*translateY\(-18px\) scale\(0\.93\)/);
  ...
  assert.match(globals, /\.favorite-toast-retiring\s*\{\s*animation:\s*favorite-toast-retire 320ms/);
