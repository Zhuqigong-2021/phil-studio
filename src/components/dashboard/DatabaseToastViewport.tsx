...
import {
  DATABASE_TOAST_EVENT,
  type DatabaseToastDetail,
...
export default function DatabaseToastViewport() {
  ...
  useEffect(() => {
    const showToast = (event: Event) => {
      dispatch({ type: "show", detail: (event as CustomEvent<DatabaseToastDetail>).detail });
    };
    window.addEventListener(DATABASE_TOAST_EVENT, showToast);
    return () => window.removeEventListener(DATABASE_TOAST_EVENT, showToast);
  }, []);

  useEffect(() => {
    if (!toast) return;
    const timer = window.setTimeout(() => dispatch({ type: "dismiss", id: toast.id }), DISMISS_AFTER_MS);
    return () => window.clearTimeout(timer);
  }, [toast]);

  useEffect(() => {
    if (!retiringToast) return;
    const timer = window.setTimeout(() => dispatch({ type: "retired", id: retiringToast.id }), RETIRE_AFTER_MS);
    return () => window.clearTimeout(timer);
  }, [retiringToast]);
      ...
      {toast && (() => {
        ...
        return (
          <div
            ...
            onKeyDown={(event) => {
              if (event.key === "Enter" || event.key === " ") dispatch({ type: "dismiss", id: toast.id });
            }}
